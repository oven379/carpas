...
export const HISTORY_DRAFT_DET_HINT = {
  ...
  textDetailing:
    'Черновик отображается в списке с меткой и доступен только вам в кабинете, пока вы не сохраните визит. До сохранения данные не становятся обычной записью в истории у владельца. Кнопка «Назад» в форме может сохранить черновик и закрыть редактирование.',
}
...
export function formOwnerVisitAdviceHintText() {
  return `Необязательно. До ${VISIT_OWNER_CARE_ADVICE_MAX_NONSPACE} символов без учёта пробелов (пробелы можно ставить свободно). Сохраняется в записи визита; в блоке «Совет» на карточке авто показывается текст из последнего по дате визита.`
}
